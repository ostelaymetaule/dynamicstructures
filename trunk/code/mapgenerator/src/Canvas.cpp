...
bool  Canvas::addSurface(Ogre::Vector2& startPosition, const char* systemType, bool enabled, Ogre::Real speed)
{
	Surface* newSurface; 

	SurfaceProperties props;
	props.type= SystemType::BLAAT;
	props.mCellObjectName= systemType;

// properties


	newSurface= (Surface*)mSystemFactory->spawnSystem(props,startPosition); 
	mSurfaces.push_back(newSurface);
	//newSurface = new Surface("Cell_system_" +StringConverter::toString((Ogre::Real)mSurfaces.size()) +"_" +std::string(systemType),this,mSceneMgr, startPosition, systemType, enabled, speed); 
...
void Canvas::removeObject(Movable2DObject* object)
{
	...
	if (object!=0)
	{
		...
		switch(object->type)
		{
			case Object2DType::SurfaceType:
				cs= static_cast<Surface*>(object);  
			
				itr = std::find(mSurfaces.begin() , mSurfaces.end() , cs);
				delete (*itr);
				(*itr)=0;
				mSurfaces.erase(itr); 
				break;
...
for (systemItr=mSurfaces.begin(); systemItr!=mSurfaces.end();systemItr++)
{
...
{
	Surface* surface = (Surface*)mCursor->getCurrentSelection(); 
