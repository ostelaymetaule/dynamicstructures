...
VoronoiDiagramGenerator::~VoronoiDiagramGenerator()
{
	//Ogre->LogManager->getSingletonPtr()->logMessage("voronoi");
	reset();
...
bool VoronoiDiagramGenerator::generateVoronoi(std::vector<Cell*>& vertexList)
{
	cleanup();
	cleanupEdges();
...
float maxY =1000;
float minDist =20;
bool genVertexInfo=true;


	minDistanceBetweenSites = minDist;

	nsites=vertexList.size();
	plot = 0;
	triangulate = 0;	
	debug = 1;
	sorted = 0; 
...
*/
	...
	xmin = vertexList[0]->getPosition().x;
	ymin = vertexList[0]->getPosition().y;
	xmax = vertexList[0]->getPosition().x;
	ymax = vertexList[0]->getPosition().y;
	...
	{
		sites[i].coord.x = (*itr)->getPosition().x;
		sites[i].coord.y = (*itr)->getPosition().y;
		sites[i].sitenbr = i;
		sites[i].refcnt = 0;

		if((*itr)->getPosition().x < xmin)
			xmin = (*itr)->getPosition().x;
		else if( (*itr)->getPosition().x > xmax)
			xmax = (*itr)->getPosition().x;

		if((*itr)->getPosition().y < ymin)
			ymin = (*itr)->getPosition().y;
		else if((*itr)->getPosition().y > ymax)
			ymax = (*itr)->getPosition().y;
