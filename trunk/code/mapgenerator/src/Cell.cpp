...
Cell::Cell(std::string name, unsigned int id,Surface* system, Ogre::SceneManager* sceneMgr, Object2DProperties* properties, Canvas* canvas, Ogre::Vector2 position, bool enabled):
	...
	if (this->isEnabled())
	{
		...
		if (mCellChild!=0)
		{
			if ((mCellChild->getPosition()- mPos).length() > (mScale * (double)mNeighbourDistance) )
			{
				...
				newCell->enable(true);
				newCell->setSurface(this->mSystem);
...
CellProperties  Cell::retrieveProperties(Surface* system)
{
...
void Cell::setSurface(Surface* Surface)
{
	mSystem = Surface;
